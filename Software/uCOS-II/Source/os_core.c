...
void  OS_TaskStat (void *p_arg)
{
    ...
    OS_EXIT_CRITICAL();
    for (;;) {
        TaskID = OS_TASK_STAT_PRIO;
        OS_ENTER_CRITICAL();
        OSIdleCtrRun = OSIdleCtr;                /* Obtain the of the idle counter for the past second */
        OSIdleCtr    = 0uL;
        OS_EXIT_CRITICAL();
        OSCPUUsage   = (INT8U)(100uL - OSIdleCtrRun/10/ OSIdleCtrMax); // Division 10 because User Define Statstics Task scantime 1sec.
